namespace chocolatey.infrastructure.app.nuget
{
    ...
    public class ChocolateyDataServicePackageRepository :
        ...
        IWeakEventListener,
        IChocolateyServiceBasedRepository
    {
        ...
        private const string GetMessageOfTheDaySvcMethod = "GetMessageOfTheDay";
        ...
        public ChocolateyDataServicePackageRepository(Uri serviceRoot)
            : this(new HttpClient(serviceRoot))
        {
        }

        public ChocolateyDataServicePackageRepository(IHttpClient client)
            : this(client, new PackageDownloader())
        {
        }

        public ChocolateyDataServicePackageRepository(IHttpClient client, IPackageDownloader packageDownloader)
        {
            if (client == null)
            {
        ...
        private void OnReadingEntity(object sender, ReadingWritingEntityEventArgs e)
        {
            ...
            //TODO RR THIS IS WHERE TO PICKUP
            //package.Context = Context;
            //package.Downloader = _packageDownloader;

            DataServicePackageRepository repo;
        }
        ...
        public IEnumerable<IPackage> FindPackagesById(string packageId)
        {
            try
            {
                ...
                {
                    // If there's no search method then we can't filter by target framework
                    return FindPackagesByIdCore(this, packageId);
                }
        ...
        /// <summary>
        /// Finds the packages by id - this repeats the internal PackageRepositoryExtensions.FindPackagesByIdCore
        /// </summary>
        /// <param name="repository">The repository.</param>
        /// <param name="packageId">The package identifier.</param>
        /// <returns></returns>
        private static IEnumerable<IPackage> FindPackagesByIdCore(IPackageRepository repository, string packageId)
        {
            var cultureRepository = repository as ICultureAwareRepository;
            packageId = packageId.ToLower(cultureRepository != null ? cultureRepository.Culture : CultureInfo.CurrentCulture);
        ...
        public IEnumerable<string> GetMessageOfTheDay()
        {
            try
            {
                if (!Context.SupportsServiceMethod(GetMessageOfTheDaySvcMethod))
                {
                    return Enumerable.Empty<string>();
                }

                // Create a query for the search service method
                var query = Context.CreateQuery<string>(GetMessageOfTheDaySvcMethod);
                return new SmartDataServiceQuery<string>(Context, query);
            }
            catch (Exception ex)
            {
                var message = string.Format(
